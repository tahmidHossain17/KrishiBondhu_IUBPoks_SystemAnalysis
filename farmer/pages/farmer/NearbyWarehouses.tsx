...
const NearbyWarehouses = () => {
  ...
  const handleBooking = async (warehouseId: string) => {
    if (!bookingDate || !bookingData.quantity || !bookingData.cropType || !profile) {
      toast({
    ...
    try {
      ...
      const estimatedCost = parseInt(bookingData.quantity) * warehouse.pricePerTon * parseInt(bookingData.duration);

      const result = await WarehouseAPI.createStorageBooking({
        ...
        storageType: bookingData.storageType || 'General Storage'
      });

      if (result.success) {
        toast({
          title: "Booking Request Sent!",
          description: `Your booking request (${result.data?.referenceNumber}) has been sent to the warehouse for confirmation.`,
        });
        ...
        toast({
          ...
          description: result.error?.message || "Failed to create booking request",
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <Button 
                          ...
                          onClick={() => {
                            ...
                            toast({
                              ...
                              description: `Selected ${warehouse.name} for booking. Please fill in the booking details.`,
                            });
                          }}
                          className={selectedWarehouse?.id === warehouse.id ? "bg-green-600 hover:bg-green-700" : ""}
                        >
                          <Plus className="w-4 h-4 mr-2" />
                          {selectedWarehouse?.id === warehouse.id ? "Selected" : "Book Storage"}
                        </Button>
                      </div>
                    </div>
                  </CardContent>
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-fresh sticky top-4" data-booking-form>
              <CardHeader>
                <CardTitle className="flex items-center">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <Button 
                    ...
                    disabled={!selectedWarehouse || !bookingDate || !bookingData.quantity || !bookingData.cropType}
                  >
                    {!selectedWarehouse ? "Select a Warehouse First" : "Send Booking Request"}
                  </Button>
                </div>
              </CardContent>
            </Card>
