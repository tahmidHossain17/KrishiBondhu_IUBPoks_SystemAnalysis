...
const FarmerDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (farmerProfile) {
        ...
        // Fetch warehouse storage data for farmer's products
        if (products.length > 0) {
          const { data: inventoryData, error: inventoryError } = await supabase
            .from('inventory')
            .select('quantity_stored')
            .in('product_id', products.map(p => p.id));

          if (inventoryError) {
            console.error('Inventory error:', inventoryError);
          } else {
            warehouseStorage = inventoryData?.reduce((sum, inv) => sum + (inv.quantity_stored || 0), 0) || 0;
          }
        }
      ...
      const recentActivities = [
        ...
        // Order activities
        ...orders.slice(0, 2).map(orderItem => ({
          type: "order",
          title: "New order received",
          description: `${orderItem.quantity} units of ${orderItem.products?.name || 'product'}`,
          time: new Date(orderItem.created_at).toLocaleTimeString(),
          icon: ShoppingCart,
  ...
  const quickStats = [
    ...
    },
    {
      ...
      value: dashboardData.quickStats.warehouseStorage > 0 ? `${dashboardData.quickStats.warehouseStorage.toFixed(1)} tons` : "0 tons",
      icon: Warehouse,
      color: "text-blue-600"
    },
    {
      ...
      <div className="space-y-6">
        ...
        {dashboardData.products.length > 0 && (
          <Card className="shadow-fresh">
            ...
            <CardContent>
              <div className="space-y-4">
                {dashboardData.products.slice(0, 3).map((product, index) => (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {product.available_quantity} {product.unit} • ৳{product.price}/{product.unit}
                        </p>
                      </div>
                    </div>
